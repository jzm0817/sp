...
```c
const int * a;         // a 指向的整型数据不可修改，但指针的内容(地址)可修改
int const * a;         // 同上
int * const a;         // 指针的内容(地址)不可修改，指向的数据可修改
const int const * a;   // 指针的内容(地址)不可修改，指向的数据也不可修改

```

**const 的作用是告诉编译器这个数据是常量，因此不能出现在赋值号的左侧，并不代表无法修改这个数据的值，起到类似 read only 的提示作用。**
```c
const int a = 20;
a = 2;
```
...
![](pic/Screenshot%20from%202022-07-26%2014-02-33.png)
...
*ptr0 = 2;
```
![](pic/Screenshot%20from%202022-07-26%2014-06-25.png)

原因在于，ptr0 所指对象并没有用 const 关键字修饰，编译时会把 a 的 const 关键字舍弃，因此结果就好像通过指针改变了常量的值，而实际上此时的 a 并非常量。

[code](code/test1/t3.c)

若要通过指针访问常量 a,在声明时要强调指针所指向的数据是不可修改的。
...
typedef 允许为各种数据类型定义新名字，只需将这个关键字写在声明前。

```c
typedef char *ptr_char;   // ptr_char means char *
ptr_char a;
```
ptr_char 表示 char 类型的指针。ptr_char 是 char * 类型的别称或称标识符。

\#define 无法正确处理指针类型

```c
#define ptr_char_d char *
ptr_char_d a, b;    //  a: char *   b: char
```

a 可正确声明为指针，b 被声明为 char 类型。
...
指针中所存放的是指针所指向的地址。利用 \* (解引用操作符)可以查看地址所存放的数据的值。
...
![](pic/Screenshot%20from%202022-07-26%2014-54-24.png)

变量包含了该序列内容为 0 或 1 的位，它们可以被解释为整数，也可被解释为浮点数，这取决于它们被使用的方式。具体被解释的方式依据不同的指令(整型指令或浮点型指令)。值的类型并非值本身所固有的一种特性，而是取决于它的使用方式。
...
*100 = 25;
```
非法，因为字面值 100 的类型是整型，而间接访问操作只能作用于指针类型的表达式。

```c
*(int * 100) = 25;
```

这个技巧唯一有用之处就是访问内存中的某些特定的位置，它并不是用于访问某个变量，而是访问硬件本身。
